...
mysqlImportForeignSchema(ImportForeignSchemaStmt *stmt, Oid serverOid)
{
	...
	appendStringInfo(&buf,
					 ...
					 "    WHEN c.DATA_TYPE = 'smallint' AND c.COLUMN_TYPE ~ 'unsigned$' THEN 'integer'"
					 ...
					 "    WHEN c.DATA_TYPE = 'int unsigned' OR (c.DATA_TYPE = 'int' AND c.COLUMN_TYPE ~ 'unsigned$') THEN 'bigint'"
					 "    WHEN c.DATA_TYPE = c.DATA_TYPE = 'bigint' AND c.COLUMN_TYPE = 'bigint(20) unsigned' THEN 'numeric(20)'"
					 "    WHEN c.DATA_TYPE = 'bigint unsigned' OR (c.DATA_TYPE = 'bigint' AND c.COLUMN_TYPE ~ 'unsigned$') THEN 'numeric'"
